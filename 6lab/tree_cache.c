...
/*
	...
	int root_size = tree->root_size;
	if(retinfo.height > log_alfa(tree->alfa, root_size)){ // log(1/alfa, size(tree))
		Node* start = find_key_node(tree->root, data->key);
		...
		if(goat_par){
			if(scapegoat == goat_par->left){
				goat_par->left = insert_rebuild_tree(head, goat_par, goat_size);
			}else{
				goat_par->right = insert_rebuild_tree(head, goat_par, goat_size);
			}
		}else{
			tree->root = insert_rebuild_tree(head, NULL, goat_size);
		}
...
/*
	...
	int root_size = tree->root_size;
	if(retinfo.height > log_alfa(tree->alfa, root_size)){ // log(1/alfa, size(tree))
		Node* start = find_age_node(tree->root, data->age);
		Node* scapegoat = find_scapegoat(tree->alfa, start);
		int goat_size = size_node(scapegoat);
		Node* head = tree_to_list(scapegoat);
		Node* ptr = head;
...
/*
	...
	if(root_size < tree->alfa * tree->max_size){
		...
		tree->root = insert_rebuild_tree(head, NULL, root_size);
